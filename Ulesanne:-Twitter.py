...
def find_fastest_growing(tweets: list) -> Tweet:
    ...
    fastest=0
    for tweet in tweets:
        spreads_fastest_in_life=tweet.retweets/tweet.time
        if spreads_fastest_in_life > fastest:
            fastest=spreads_fastest_in_life 
            fastest_tweet=tweet
...
def sort_by_popularity(tweets: list) -> list:
    ...
    sort_tweet = sorted(tweets, key=popularity_key)
    return sort_tweet
...
def filter_by_hashtag(tweets: list, hashtag: str) -> list:
    ...
    """
    filter_tweets=list(filter(lambda tweet: hashtag in tweet.content, tweets))
...
def sort_hashtags_by_popularity(tweets: list) -> list:
    ...
    sorted_hashtag = sorted(hashtag_popularity.items(), key=lambda item: (-item[1], item[0]))

    return [hashtag for hashtag, _ in sorted_hashtag]
